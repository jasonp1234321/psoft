...
public final class RatPoly {
    ...
    public RatPoly negate() {
        ...
        RatNum[] negCoeffs = new RatNum[coeffs.length];
        for (int i = 0; i < coeffs.length; i++) {
            negCoeffs[i] = coeffs[i].negate();
        }
        return new RatPoly(negCoeffs);
    }
    ...
    public RatPoly add(RatPoly p) {
        ...
        RatNum[] sumCoeffs = new RatNum[maxDegree + 1];
        Arrays.fill(sumCoeffs, RatNum.ZERO);

        for (int i = 0; i <= this.degree; i++) {
            sumCoeffs[i] = sumCoeffs[i].add(this.coeffs[i]);
        }
        for (int i = 0; i <= p.degree; i++) {
            sumCoeffs[i] = sumCoeffs[i].add(p.coeffs[i]);
        }

        sumCoeffs = trimCoeffs(sumCoeffs);
        
        return new RatPoly(sumCoeffs);
    }
    
    private static RatNum[] trimCoeffs(RatNum[] coeffs) {
        int newDegree = coeffs.length - 1;
        while (newDegree > 0 && coeffs[newDegree].equals(RatNum.ZERO)) {
            newDegree--;
        }
     
        if (newDegree == 0 && coeffs[0].equals(RatNum.ZERO)) {
            return new RatNum[0];  
        }
        return Arrays.copyOf(coeffs, newDegree + 1);
    }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        RatNum[] productCoeffs = new RatNum[newDegree + 1];
        Arrays.fill(productCoeffs, RatNum.ZERO);
        
        for (int i = 0; i <= this.degree; i++) {
            for (int j = 0; j <= p.degree; j++) {
                productCoeffs[i + j] = productCoeffs[i + j].add(this.coeffs[i].mul(p.coeffs[j]));
            }
        }
        
        return new RatPoly(productCoeffs);
    }
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatNum[] quotientCoeffs = new RatNum[this.degree - p.degree + 1];
        Arrays.fill(quotientCoeffs, RatNum.ZERO);
        
        RatPoly remainder = new RatPoly(this.coeffs);
        
        while (remainder.degree >= p.degree) {
            RatNum leadingCoeff = remainder.coeffs[remainder.degree].div(p.coeffs[p.degree]);
            int exponent = remainder.degree - p.degree;
            
            RatNum[] termCoeffs = new RatNum[exponent + 1];
            Arrays.fill(termCoeffs, RatNum.ZERO);
            termCoeffs[exponent] = leadingCoeff;
            
            RatPoly term = new RatPoly(termCoeffs);
            quotientCoeffs[exponent] = leadingCoeff;
            
            remainder = remainder.sub(term.mul(p));
        }
        
        return new RatPoly(quotientCoeffs);
    }
    ...
    public double eval(double d) {
        ...
        for (int i = this.degree; i >= 0; i--) {
            result = result * d + this.coeffs[i].doubleValue();
        }
    ...
    public RatPoly differentiate() {
        ...
        for (int i = 1; i <= this.degree; i++) {
            newCoeffs[i - 1] = this.coeffs[i].mul(new RatNum(i)); 
        }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        for (int i = 0; i <= this.degree; i++) {
            newCoeffs[i + 1] = this.coeffs[i].div(new RatNum(i + 1));
        }
